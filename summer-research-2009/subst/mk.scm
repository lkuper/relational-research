...
(module mk 
  (lambdaf@ lambdag@
   ...
   ext-s-init size-s
   run* rhs lhs var-1 var-2 var-2-skew var-2-skew-def var var? 
...
(define walk-bst
 (lambda (v s)
   (cond
     ((var? v)
      (let ((a (bst:lookup (bst:hash (vector-ref v 1)) s)))
        (cond
...
(define ext-s-bst
 (lambda (x v s)
   (bst:ext (bst:hash (vector-ref x 1)) v s)))
(define ext-s-skew
  (lambda (x v s)
...
(define-syntax var
  (syntax-rules ()
    [(_ x s) (choose-var (var-1 x) (var-2 x) (var-2-skew x)
                         (var-2-skew-def x s))]))
